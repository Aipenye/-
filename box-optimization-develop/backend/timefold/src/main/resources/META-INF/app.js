...
const containerDateFormatter = JSJoda.DateTimeFormatter.ofPattern('d MMM').withLocale(JSJodaLocale.Locale.ENGLISH);
...
function renderScheduleByRoom(schedule) {
    ...
    let content = `<div class="d-flex flex-column"><div><h5 class="card-title mb-1">${container.name}</h5></div>`;
    const containerData = {
        id: container.id,
        content: content,
        treeLevel: 1,
        nestedLevels: [...container.airCubes.map(b => b.id)]
    };
    byRoomGroupData.add(containerData);
    container.airCubes.forEach(air => byRoomGroupData.add({
        id: air.id,
        content: `Bed (${air.x}, ${air.y}, ${air.z})`,
        treeLevel: 2
    }));

    const airMap = new Map();
    container.airCubes.forEach(a => airMap.set(a.id, a));

    $.each(schedule.boxes, (_, box) => {
        if (box.airCube == null) {
            unassignedJobsCount++;
            const unassignedPatientElement = $(`<div class="card-body p-2"/>`)
                .append($(`<h5 class="card-title mb-1"/>`).text(`${box.name}`))
                .append($(`<p class="card-text ms-2 mb-0"/>`).text(`${JSJoda.LocalDate.parse(box.arrivalDate)
                    .until(JSJoda.LocalDate.parse(box.departureDate), JSJoda.ChronoUnit.DAYS)} day(s)`))
                .append($(`<p class="card-text ms-2 mb-0"/>`).text(`Arrival: ${box.arrivalDate}`))
                .append($(`<p class="card-text ms-2 mb-0"/>`).text(`Departure: ${box.departureDate}`));
            ...
            byRoomItemData.add({
                id: box.id,
                group: box.id,
                start: box.arrivalDate,
                end: box.departureDate,
            ...
            const color = box.patientGender == '#729FCF';
    ...
    if (unassignedJobsCount === 0) {
        unassignedPatients.append($(`<p/>`).text(`There are no unassigned boxes.`));
    }

    const arrivalDates = schedule.boxes.map(s => s.arrivalDate);
    const departureDates = schedule.boxes.map(s => s.departureDate);
...
function importLocalFile() {
    ...
    reader.addEventListener("load", function () {
        ...
        try {
            ...
            console.error(error);
            showSimpleError("Failed loading a airCube plan.\nCheck if the content of the file represents a valid airCube plan.");
        }
