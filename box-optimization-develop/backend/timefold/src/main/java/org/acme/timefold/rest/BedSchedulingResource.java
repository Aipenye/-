...
import org.acme.timefold.rest.exception.ErrorInfo;
import org.acme.timefold.rest.exception.ScheduleSolverException;
...
public class BedSchedulingResource {
    ...
    private final SolverManager<BoxPlan, String> solverManager;
    private final SolutionManager<BoxPlan, HardSoftScore> solutionManager;
    ...
    @Inject
    public BedSchedulingResource(SolverManager<BoxPlan, String> solverManager,
                                 SolutionManager<BoxPlan, HardSoftScore> solutionManager) {
    ...
    public String solve(BoxPlan problem) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("analyze")
    public ScoreAnalysis<HardSoftScore> analyze(BoxPlan problem,
    ...
    @APIResponses(value = {
            @APIResponse(responseCode = "200", description = "The best solution of the schedule so far.",
                    content = @Content(mediaType = MediaType.APPLICATION_JSON,
                            schema = @Schema(implementation = BoxPlan.class))),
        ...
        private void schedule_to_file(BoxPlan schedule) {
                try {
                        FileWriter myWriter = new FileWriter("boxSets/out/solution.txt");
                        myWriter.write(schedule.to_data());
                        myWriter.close();
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("{jobId}/status")
    public BoxPlan getStatus(
            @Parameter(description = "The job ID returned by the POST method.") @PathParam("jobId") String jobId) {
        BoxPlan schedule = getScheduleAndCheckForExceptions(jobId);
        SolverStatus solverStatus = solverManager.getSolverStatus(jobId);
        return new BoxPlan(schedule.getScore(), solverStatus);
    }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("{jobId}")
    public BoxPlan terminateSolving(
            @Parameter(description = "The job ID returned by the POST method.") @PathParam("jobId") String jobId) {
        solverManager.terminateEarly(jobId);
        return getSchedule(jobId);
    }

    private BoxPlan getScheduleAndCheckForExceptions(String jobId) {
    ...
    private record Job(BoxPlan schedule, LocalDateTime createdAt, Throwable exception) {

        static Job ofSchedule(BoxPlan schedule) {
